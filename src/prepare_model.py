...
CHANGE_SITE_POS = False
USE_SPRINGLENGTH = False
...
def load_tendons(root, tendon_path='assets/tendons'):
    ...
    for tendon in tendon_files:
        ...
        if MODIFY_TEMPLATE:
            ...
            if USE_SPRINGLENGTH:
                loc_attrib['springlength'] = "{{" + f'{tendon_name}.springlength' + '}}'
        ...
        if USE_SPRINGLENGTH:
            yaml_dict[tendon_name]['springlength'] = f'{dist}'
...
def generate_convex_sub_meshes(root):
    for name in names:
        # 1. load the meshes
        mesh = trimesh.load(f'{STL_PATH}/{name}.stl')
        # 2. perform decomposition
        convex_meshes = trimesh.interfaces.vhacd.convex_decomposition(
            mesh,
            resolution=10000000,
        )

        if type(convex_meshes) != list:
            convex_meshes = [convex_meshes]

        # 3. add all submeshes
        for index, convex_mesh in enumerate(convex_meshes):

            # first save the mesh
            loc_name = f'{name}_{index}'
            convex_mesh.export(f'{STL_PATH}/{STL_SUB_PATH}/{loc_name}.stl')

            # second add mesh to model assets
            found = False
            for elem in root[2]:
                if elem.attrib['name'] == loc_name:
                    found = True

            if not found:
                loc_attrib = {
                    'name': loc_name,
                    'file': f'/{STL_SUB_PATH}/{loc_name}.stl',
                }
                root[2].makeelement('mesh', loc_attrib)
                ET.SubElement(root[2], 'mesh', loc_attrib)

            # third link the mesh in the model
            recursive_mesh(root[3], name, loc_name)
